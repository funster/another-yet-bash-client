...
import ru.aim.anotheryetbashclient.async.commands.ContextCommand;
...
public class AsyncService extends IntentService {
    ...
    public static String TASK_FINISHED_ACTION = AsyncUtils.ACTION_PREFIX + "TASK_FINISHED_ACTION";
    ...
    void beforeExecute(ISimpleCommand command) {
        if (command instanceof ContextCommand) {
            ((ContextCommand) command).setContext(getApplicationContext());
        }
    }

    void afterExecute(ISimpleCommand command) {
        if (command instanceof ContextCommand) {
            ((ContextCommand) command).setContext(null);
        }
    }
